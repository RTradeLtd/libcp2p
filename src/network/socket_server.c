...
socket_server_t *new_socket_server(thread_logger *thl, socket_server_config_t config) {
    ...
    if (tcp_set == true) {
        ...
        if (errno != 0) {
            thl->logf(thl, 0, LOG_LEVELS_ERROR, "failed to start listening on tcp socket with error %s", strerror(errno));
            goto EXIT;
        }
    ...
    if (udp_set == true) {
        ...
        if (errno != 0) {
            thl->logf(thl, 0, LOG_LEVELS_ERROR, "failed to start listening on udp socket with error %s", strerror(errno));
            goto EXIT;
        }
    ...
    socket_server_t *server = calloc(sizeof(socket_server_t), sizeof(socket_server_t));
    if (server == NULL) {
