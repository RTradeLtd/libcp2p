...
int get_new_socket(thread_logger *thl, addr_info *bind_address,
    ...
    if (listen_socket_num <= 0) {
        thl->log(thl, 0, "socket creation failed", LOG_LEVELS_ERROR, __FILENAME__,
                 __LINE__);
        return -1;
    }
    int one;
            ...
            case REUSEADDR:
                ...
                if (rc != 0) {
                    thl->log(thl, 0, "failed to set socket reuse addr",
                             LOG_LEVELS_ERROR, __FILENAME__, __LINE__);
                    return -1;
                }
...
bool recv_or_send_failed(thread_logger *thl, int rc) {
    switch (rc) {
        ...
        case -1:
            if (thl != NULL) {
                thl->logf(thl, 0, LOG_LEVELS_DEBUG, __FILENAME__, __LINE__,
