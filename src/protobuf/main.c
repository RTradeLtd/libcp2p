...
int main(int argc, char **argv) {
  ...
  while (pos < numBytes) {
    ...
    if (protobuf_decode_field_and_type(&buffer[pos], numBytes - pos, &field_no,
                                       &wire_type, &bytes_read) == 0) {
      printf("Unexpected return value from protobuf_decode_field_and_type at "
             "pos %lu\n",
             pos);
      return 0;
    }
    printf("Field counter: %d at position %lu, Field Number: %d, Wire Type: %s",
           fieldCounter, pos, field_no, getWireType(wire_type));
    pos += bytes_read;
    // read the value
    switch (wire_type) {
    case (WIRETYPE_VARINT): {
