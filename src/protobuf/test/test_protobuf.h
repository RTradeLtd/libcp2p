 ...
 * }
 *
 * int Test1_protobuf_encode(struct Test1* incoming, unsigned char* buffer,
 * size_t max_buffer_length, size_t* bytes_written); int
 * Test1_protobuf_decode(unsigned char* buffer, size_t buffer_length, struct
 * Test1** output);
 */
#include <stdlib.h>
#include <string.h>
...
int test_rightshift() {
    ...
    if (bytes_written != 2) {
        printf("Incorrect number of bytes written. Expected 2, but got "
               "%d\n",
               retVal);
        return 0;
    }
...
int test_write_simple() {
    ...
    if (bytes_written != 3) {
        printf("Incorrect number of bytes written. Expected 3, but got "
               "%d\n",
               retVal);
        return 0;
    }
...
int test_write_negative() {
    ...
    if (bytes_written != 11) {
        printf("Incorrect number of bytes written. Expected 11, but "
               "got %d\n",
               retVal);
        return 0;
    }
    ...
    if (pos0 != 8 || pos1 != 234 || pos2 != 254 || pos3 != 255 || pos4 != 255 ||
        pos5 != 255 || pos6 != 255 || pos7 != 255 || pos8 != 255 ||
        pos9 != 255 || pos10 != 1) {
        printf("Results not as expected\n");
        return 0;
    }
...
int test_write_string() {
    ...
    if (bytes_written != 9) {
        printf("Incorrect number of bytes written. Expected 9, but got "
               "%d\n",
               retVal);
        return 0;
    }
    ...
    for (int i = 0; i < 9; i++) {
        if (expected[i] != buffer[i]) {
            correct = 0;
            printf("Incorrect value at position %d... Expected "
                   "%02x but received "
                   "%02x.\n",
