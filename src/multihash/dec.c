...
/*!
  * @author bonedaddy
  * @note used to silence `ISO C forbids specifying range of elements to initialize `
  * @note used to silence `initialized field overwritten`
*/
#pragma GCC diagnostic ignored "-Wpedantic"
#pragma GCC diagnostic ignored "-Woverride-init"

static const long  value_table[] = {
	[0 ... 255] = -1,
...
unsigned long long bin_to_dec(unsigned const char* bin)
{
	...
	int count = 0;
	while (bin[count] != '\0') {
		if (bin[count] == '1') dec = dec * 2 + 1;
		else if (bin[count] == '0') dec *= 2;
		count++;
	}
	return dec;
}
