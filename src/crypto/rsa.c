 ...
 * NOTE: the stream starts from the right. So there could be a lot of padding in
 * front. Pay attention to the returned size to cut the padding.
 * @param rsa the rsa key to encode
 * @param buf where to put the bytes
 * @param size the max size of the buffer. The actual size used is returned in
 * this value
 * @returns true(1) on success, else 0
 */
static int libp2p_crypto_rsa_write_private_key_der(mbedtls_rsa_context *rsa,
                                                   unsigned char *buf,
                                                   size_t *size) {
    int ret;
 ...
 */
static int libp2p_crypto_rsa_write_public_key_der(mbedtls_pk_context *key,
    ...
    *--c = 0;
    ...
    MBEDTLS_ASN1_CHK_ADD(len,
                         mbedtls_asn1_write_tag(&c, buf, MBEDTLS_ASN1_BIT_STRING));
...
/***
 * Generate an RSA keypair of a certain size, and place the results in the
 * struct
 * @param private_key where to put the results
 * @param num_bits_for_keypair the number of bits for the key, 1024 is the
 * minimum
...
int libp2p_crypto_rsa_generate_keypair(struct RsaPrivateKey *private_key,
    ...
    if (mbedtls_rsa_gen_key(&rsa, mbedtls_ctr_drbg_random, &ctr_drbg,
                            (unsigned int)num_bits_for_keypair, exponent) != 0) {
        goto exit;
    }
...
exit:
    ...
    if (buffer != NULL)
        free(buffer);
    if (retVal == 0) {
        // anything allocated should be cleaned up, as we're erroring
        // out
...
int libp2p_crypto_rsa_private_key_fill_public_key(
    ...
    mbedtls_pk_init(&ctx);
    mbedtls_pk_parse_key(&ctx, (unsigned char *)private_key->der,
                         private_key->der_length, NULL, 0);
...
struct RsaPrivateKey *libp2p_crypto_rsa_rsa_private_key_new(void) {
    struct RsaPrivateKey *out =
        (struct RsaPrivateKey *)malloc(sizeof(struct RsaPrivateKey));
    if (out != NULL) {
        out->D = 0;
        out->DP = 0;
        out->DQ = 0;
 ...
 * the size of the private key (i.e. 2048 bit key can store a sig in 256 bytes)
...
int libp2p_crypto_rsa_sign(struct RsaPrivateKey *private_key, const char *message,
    ...
    if (mbedtls_pk_parse_key(&private_context, der, private_key->der_length, NULL,
                             0) != 0)
        goto exit;
    ...
    *result = (unsigned char *)malloc(*result_size);
    // sign
    retVal = mbedtls_rsa_rsassa_pkcs1_v15_sign(ctx, mbedtls_ctr_drbg_random,
                                               &ctr_drbg, MBEDTLS_RSA_PRIVATE,
                                               MBEDTLS_MD_SHA256, 32, hash, *result);
    // retVal = mbedtls_rsa_private(ctx, mbedtls_ctr_drbg_random, &ctr_drbg,
    // hash, result);
    if (retVal != 0) {
        retVal = 0;
        goto exit;
 ...
 *@returns true(1) if the signature matches the SHA2-256 hash of message,
 *false(0) otherwise
 */
int libp2p_crypto_rsa_verify(struct RsaPublicKey *public_key,
    ...
    mbedtls_pk_parse_public_key(&public_context, (unsigned char *)public_key->der,
                                public_key->der_length);

    mbedtls_rsa_context *ctx = mbedtls_pk_rsa(public_context);
    mbedtls_ctr_drbg_context ctr_drbg;
    mbedtls_ctr_drbg_init(&ctr_drbg);

    int retVal = mbedtls_rsa_rsassa_pkcs1_v15_verify(
        ctx,  // the rsa public key has to be in the context
        NULL, // random number generator, but not needed because this is
              // not a private key
