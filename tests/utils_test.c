...
void *test_thread_log(void *data) {
    ...
    thl->log(thl, 0, "this is an info log", LOG_LEVELS_INFO, __FILE__, __LINE__);
    thl->logf(thl, 0, LOG_LEVELS_INFO, __FILE__, __LINE__, "%s\t%s", "one", "two");

    thl->log(thl, 0, "this is a warn log", LOG_LEVELS_WARN, __FILE__, __LINE__);
    thl->logf(thl, 0, LOG_LEVELS_WARN, __FILE__, __LINE__, "%s\t%s", "one", "two");

    thl->log(thl, 0, "this is an error log", LOG_LEVELS_ERROR, __FILE__, __LINE__);
    thl->logf(thl, 0, LOG_LEVELS_ERROR, __FILE__, __LINE__, "%s\t%s", "one", "two");

    thl->log(thl, 0, "this is a debug log", LOG_LEVELS_DEBUG, __FILE__, __LINE__);
    thl->logf(thl, 0, LOG_LEVELS_DEBUG, __FILE__, __LINE__, "%s\t%s", "one", "two");
...
void *test_file_log(void *data) {
    ...
    fhl->thl->log(fhl->thl, fhl->file_descriptor, "this is an info log", LOG_LEVELS_INFO, __FILE__, __LINE__);
    fhl->thl->logf(fhl->thl, fhl->file_descriptor, LOG_LEVELS_INFO, __FILE__, __LINE__, "%s\t%s", "one", "two");

    fhl->thl->log(fhl->thl, fhl->file_descriptor, "this is a warn log", LOG_LEVELS_WARN, __FILE__, __LINE__);
    fhl->thl->logf(fhl->thl, fhl->file_descriptor, LOG_LEVELS_WARN, __FILE__, __LINE__, "%s\t%s", "one", "two");

    fhl->thl->log(fhl->thl, fhl->file_descriptor, "this is an error log", LOG_LEVELS_ERROR, __FILE__, __LINE__);
    fhl->thl->logf(fhl->thl, fhl->file_descriptor, LOG_LEVELS_ERROR, __FILE__, __LINE__, "%s\t%s", "one", "two");
    
    fhl->thl->log(fhl->thl, fhl->file_descriptor, "this is a debug log", LOG_LEVELS_DEBUG, __FILE__, __LINE__);
    fhl->thl->logf(fhl->thl, fhl->file_descriptor, LOG_LEVELS_DEBUG, __FILE__, __LINE__, "%s\t%s", "one", "two");
    // commenting this out seems to get rid of memleaks reported by valgrind
    // pthread_exit(NULL);
    return NULL;
}
